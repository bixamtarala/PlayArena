...
class PlayArenaApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        title: 'PlayArena',
        theme: ThemeData(useMaterial3: true, brightness: Brightness.dark, colorSchemeSeed: Colors.amber, scaffoldBackgroundColor: const Color(0xFF07131D)),
...
class HomeScreen extends StatelessWidget { const HomeScreen({super.key}); @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('PlayArena'),actions:[IconButton(onPressed:(){},icon:const Icon(Icons.notifications_none))]),body:ListView(padding:const EdgeInsets.all(18),children:[
Card(child:Padding(padding:const EdgeInsets.all(18),child:Row(children:[const CircleAvatar(radius:26,child:Icon(Icons.person)),const SizedBox(width:14),const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text('Welcome, Player',style:TextStyle(fontWeight:FontWeight.bold,fontSize:18)),Text('Free-play entertainment')])),Column(crossAxisAlignment:CrossAxisAlignment.end,children:const [Text('CHIPS',style:TextStyle(fontSize:11)),Text('10,000',style:TextStyle(color:Colors.amber,fontWeight:FontWeight.bold,fontSize:20)),Text('Admin managed',style:TextStyle(fontSize:10))])]))),
const SizedBox(height:18),Text('Choose a game',style:Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight:FontWeight.bold)),const SizedBox(height:12),
GameCard(title:'Teen Patti',subtitle:'Free-play table preview',icon:Icons.style,onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const TeenPattiScreen()))),
...
const SizedBox(height:14),const Card(child:Padding(padding:EdgeInsets.all(16),child:Text('Virtual chips are admin-managed entertainment credits. They have no cash value and cannot be withdrawn, sold or redeemed.',textAlign:TextAlign.center))),
]));}
...
class ComingSoonScreen extends StatelessWidget{final String title;const ComingSoonScreen({super.key,required this.title});@override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:Text(title)),body:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[const Icon(Icons.construction,size:72,color:Colors.amber),const SizedBox(height:16),Text('$title engine is the next build',style:Theme.of(context).textTheme.titleLarge)])));}
