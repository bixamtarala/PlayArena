...
class _TeenPattiScreenState extends State<TeenPattiScreen> {
  ...
  void deal() {
    ...
    setState(() { cards = copy.sublist(0,3); bot2 = copy.sublist(3,6); bot3 = copy.sublist(6,9); pot=30; turnStake=10; seen=false; folded=false; finished=false; revealBots=false; status='Cards dealt • Your turn'; });
  }
  void seeCards()=>setState((){seen=true;status='Your hand: ${TeenPattiEngine.evaluate(cards).label}';});
  void chaal()=>setState((){pot+=turnStake;status='${seen?'Chaal':'Blind'}: $turnStake virtual chips • Pot $pot';});
  void raise()=>setState((){turnStake*=2;pot+=turnStake;status='Raise • Current stake $turnStake';});
  void fold()=>setState((){folded=true;finished=true;status='You packed • Start a new hand';});
  void showdown(){
    if(cards.isEmpty||folded)return;
    final hands=[cards,bot2,bot3]; var best=0;
    for(var i=1;i<hands.length;i++){if(TeenPattiEngine.compare(hands[i],hands[best])>0)best=i;}
    setState((){finished=true;revealBots=true;seen=true;status=best==0?'You win the preview showdown • ${TeenPattiEngine.evaluate(cards).label}':best==1?'Player 2 wins • ${TeenPattiEngine.evaluate(bot2).label}':'Player 3 wins • ${TeenPattiEngine.evaluate(bot3).label}';});
  }
  ...
  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('Teen Patti • Preview')),body:SafeArea(child:Column(children:[
    Padding(padding:const EdgeInsets.all(10),child:Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[_Bot(name:'Player 2',cards:bot2,reveal:revealBots),Column(children:[const Text('POT',style:TextStyle(fontSize:11)),Text('$pot',style:const TextStyle(color:Colors.amber,fontSize:24,fontWeight:FontWeight.bold))]),_Bot(name:'Player 3',cards:bot3,reveal:revealBots)])),
    Expanded(child:Container(margin:const EdgeInsets.all(10),decoration:BoxDecoration(color:const Color(0xFF0D5B3B),borderRadius:BorderRadius.circular(120),border:Border.all(color:Colors.amber.shade700,width:3)),child:Center(child:Padding(padding:const EdgeInsets.all(20),child:Text(status,textAlign:TextAlign.center,style:const TextStyle(fontWeight:FontWeight.bold))))),
    const _Player(name:'You',chips:'Admin managed'),const SizedBox(height:8),
    ...
    const Padding(padding:EdgeInsets.all(12),child:Text('Offline gameplay preview • No cash value • No wallet changes are performed on-device.',textAlign:TextAlign.center,style:TextStyle(fontSize:11))),
  ])));
}

class _Bot extends StatelessWidget{final String name;final List<String> cards;final bool reveal;const _Bot({required this.name,required this.cards,required this.reveal});@override Widget build(BuildContext context)=>Column(children:[const CircleAvatar(radius:18,child:Icon(Icons.person,size:18)),Text(name,style:const TextStyle(fontSize:11,fontWeight:FontWeight.bold)),if(cards.isNotEmpty)Text(reveal?TeenPattiEngine.evaluate(cards).label:'3 cards',style:const TextStyle(fontSize:10))]);}
